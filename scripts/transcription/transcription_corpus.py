...
def transcription(corpusPath, stats_auteur):
    ...
    pattern = re.compile(
        "^h[aeiouyéèà]{2}"
    )  # Tentative de règle pour les h aspirés, voir dans la boucle
    #######################################
                            ...
                            if (
                                ...
                                for vers in div.findall(
                                    ...
                                    for mot in liste_mots_vers:
                                        if "'" in mot:
                                            sep = mot.split("'")
                                            liste_mots_vers.insert(
                                                liste_mots_vers.index(mot), sep[0]
                                            )
                                            liste_mots_vers.insert(
                                                liste_mots_vers.index(mot), sep[1]
                                            )
                                    ...
                                    liste_vers_phonetique_sanscouleur.append(
                                        vers_phonetique_sanscouleur
                                    )
...
(
    ...
    liste_vers_phonetique,
    syllabes,
    liste_vers_phonetique_sans_couleur,
) = transcription(
    Path("../Corpus/Baudelaire"),
    Path("../Corpus/Baudelaire/analyse_CM_profils_baudelaire.csv"),
)
...
resultats_csv(
    ...
    liste_vers_phonetique_sans_couleur,
    syllabes,
    Path("../resultats/csv_transcriptions/baudelaire.csv"),
)
